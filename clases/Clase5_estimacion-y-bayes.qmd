...
```{r}
datos <- read.csv(here::here("R_curso_modelos", "datos", "barbera_data_fire_total_climate.csv"))
...
`prod(dpois(y, lambda)) * dgamma(lambda, alpha, beta)`
...
for (i in 1:ns) {
  ...
  d0$prior[i] <- dunif(lambda_seq[i], 0, 100) # previa plana
}
...
`prod(dpois(y, lambda = exp(alpha + beta * x)) *`

`dnorm(alpha, mu_a, sigma_a) *`

`dnorm(beta, mu_b, sigma_b)`
...
`sum(dpois(y, lambda = exp(alpha + beta * x), log = T) +`

`dnorm(alpha, mu_a, sigma_a, log = T) +`

`dnorm(beta, mu_b, sigma_b, log = T)`
